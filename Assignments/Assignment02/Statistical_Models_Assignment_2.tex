...
\end{equation}
We can factor out one division of $q$ of the entire sum
\begin{equation}
    M_X(t) = \dfrac{\pi}{q} \sum_{k=0}^{\infty} (e^t q)^{k}
\end{equation}
Using the hint, we get a geometric series if $|e^t q| < 1$, since the is a product of strictly positive numbers, we can drop the absolute value, such that
\begin{equation}
    e^t q < 1 \implies t + \ln(q) < 0 \implies t < -\ln(q) = -\ln(1-\pi). \label{geom}
\end{equation}
Using the formula for the sum of a geometric series, provided we satisfy \eqref{geom}, then

\begin{equation}
    M_X(t) = \dfrac{\pi}{q} \cdot \dfrac{e^tq}{1 - e^t q}  = \dfrac{\pi e^t}{1 - (1 - \pi)e^t}
\end{equation}
We can now use this moment generating function to find the mean and variance of $X$
\begin{equation}
...
\end{equation}
Using the chain rule, we have that the derivative of $M_{Y_n}(t)$ 
\begin{equation}
...
\end{equation}
We know the value of $M_{X_1}(t)$ from part a), thus
\begin{equation}
...
\end{equation}
Then from $b)$, we know that we can express $Y_n$ as a sum of $n$ i.i.d. geometric random variables, as such
\begin{equation}
...
\subsection*{e)}
We now wish to find the exponential disperation representation of $Y_n$ for a fixed $n$. If we take the log of the PMF of $Y_n$ and let $\beta = \binom{k-1}{n-1}$, we arrive at
\begin{equation}
    \ln(P(Y_n = k))  = k\ln(1 - \pi) + n\left(\ln(\pi) - \ln(1 - \pi)\right) + \ln(\beta)
\end{equation}
Now, we define $theta = \ln(1 - \pi)$, and $b(\theta) = -n\left(\ln(\pi) - \theta\right)$. It is usefule to note that $\theta = \ln(1 - \pi) \implies e^{\theta} = 1 - \pi \implies \pi = 1 - e^{\theta}$. Thus we can rewrite $b(\theta)$ as

\begin{equation}
    b(\theta) = -n\left(\ln(1 - e^{\theta}) - \theta\right).
\end{equation}
We then arrive at the following representation of the log PMF
\begin{equation}
    \ln(P(Y_n = k)) = k\theta - b(\theta) + \ln(\beta).
\end{equation}
This gives that the dispersion paratameter $\phi = 1$, and the canonical link function is given by
\begin{equation}
    c(k, \phi) = \ln(\beta),
\end{equation}
Thus, we see that the negative binomial distribution is a exponential dispersion model
\begin{equation}
    P(Y_n = k) = \exp\left(\dfrac{k\theta - b(\theta)}{\phi} + c(k, \phi)\right).
\end{equation}
We may find the mean by taking the derivative of $b(\theta)$
\begin{equation}
    E[Y_n] = b'(\theta) = -n\left(\dfrac{-e^{\theta}}{1 - e^{\theta}} - 1\right) = -n\left(\dfrac{-(1 - \pi)}{\pi} - 1\right) = -n\left(\dfrac{-1}{\pi} + 1 - 1 \right) = \dfrac{n}{\pi},
\end{equation}
which is consistent with our previous results. We can find the variance by taking the second derivative of $b(\theta)$, since $\phi = 1$, we have
\begin{equation}
    \text{Var}(Y_n) = b''(\theta) = -n\left(\dfrac{-e^{\theta}}{(1 - e^{\theta})^2} \right) = -n\left(\dfrac{-(1 - \pi)}{\pi^2} \right) = \dfrac{n(1 - \pi)}{\pi^2},
\end{equation}
which is also consistent with our previous results. $\square$
\subsection*{f1)}
We can use that the moment generating function of $\tilde{X}_\pi = \frac{\pi}{1 - \pi}X$ is a linear transformation of the moment generating function of $X$, such that \footnote{\url{https://en.wikipedia.org/wiki/Moment_generating_function}}
\begin{equation}
    M_{\tilde{X}_\pi}(t) = M_{X}(\dfrac{\pi}{1 - \pi}t).
\end{equation}
As such,
\begin{equation}
    M_{\tilde{X}_\pi}(t) = \dfrac{\pi e^{\frac{\pi}{1 - \pi}t}}{1 - (1 - \pi)e^{\frac{\pi}{1 - \pi}t}}.
\end{equation}
Now let
$$g(\pi) = \pi e^{t\frac{\pi}{1 - \pi}} \quad h(\pi) = 1 - (1 - \pi)e^{\frac{\pi}{1 - \pi}t},$$
then we have
$$g'(\pi) = e^{t\frac{\pi}{1 - \pi}} +\pi e^{t\frac{\pi}{1 - \pi}}\dfrac{1}{(1 - \pi)^2} \quad h'(\pi) =  -e^{\frac{\pi}{1 - \pi}t} + (1 - \pi)e^{\frac{\pi}{1 - \pi}t} \cdot \frac{1}{(1 - \pi)^2}.$$
If we use l'Hopital's rule, we have that
\begin{equation}
    \lim_{\pi \to 0} M_{\tilde{X}_\pi}(t) = \lim_{\pi \to 0} \dfrac{g'(\pi)}{h'(\pi)} = \dfrac{e^t}{1-e^t} =
\end{equation}
